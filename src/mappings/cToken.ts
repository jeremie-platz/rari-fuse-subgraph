...
export function handleBorrow(event: Borrow): void {
  ...
  // If the asset is ERC20 and not ETH
  if (entity.underlying !== EMPTY_ADDRESS) {
    let balanceCall = erc20.try_balanceOf(event.params.borrower);
    log.warning(
      "handleBorrow: trying erc20Balance: token: {} -  entity.underlying: {} - entity.id: {} - borrower: {}",
      [
    ...
    //   updateCtoken(event, entity, event.address);
      ...
      log.warning(
        "handleBorrow: erc20 balanceCall Reverted: token: {} -  entity.underlying: {} - entity.id: {} - borrower: {}",
        [
          erc20._address.toHexString(),
          entity.underlying,
          entity.id,
  ...
  // Todo:  Handle if asset is ETH
  else {
    log.debug("Asset is ETH: {}", [entity.id]);
  }
...
function updateCtoken(
  ...
  if (!_price.reverted) {
    ...
    return;
    // if the price reverts, then code below will revert as well. return out
  }
  /* 
  ...
  if (!_supplyRatePerBlock.reverted) {
    ...
    log.warning("🚨 updating supplyAPY to {} from {}", [
      convertMantissaToAPY(
